...
function Nav({ name, banner, isThemeChanged }) {
  ...
  return (
    ...
    >
      <button onClick={handleBack} className={`p-2 ${isThemeChanged ? 'bg-teal-300 hover:bg-teal-600' : 'bg-pink-300 hover:bg-pink-600'}`}
        style={{
      ...
      <h4 className="h-full p-2 font-bold text-lg">{name}'s <span className='lowercase'>{banner}</span></h4>
      <a href="http://localhost:8000/logout"
        className='p-2 bg-red-300 hover:bg-red-600 hover:text-white'
        style={{
          cursor: isThemeChanged
            ? `url('/select2.png') 2 2, pointer`
            : `url('/select1.png') 2 2, pointer`,
